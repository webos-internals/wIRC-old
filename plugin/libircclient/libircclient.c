...
void apps_ssl_info_callback(SSL *s, int where, int ret) {
	...
	if (where & SSL_CB_LOOP) {
		...
		syslog(LOG_ALERT, "SSL3 alert %s:%s:%s\n",
				str,
		...
		if (ret == 0)
			syslog(LOG_ALERT, "%s:failed in %s\n",
					str,SSL_state_string_long(s));
		else if (ret < 0) {
			syslog(LOG_ALERT, "%s:error in %s\n",
					str,SSL_state_string_long(s));
		}
	}
}
...
int irc_process_select_descriptors (irc_session_t * session, fd_set *in_set, fd_set *out_set)
{
	...
	{
		//syslog(LOG_ALERT, "We are in process_select_descriptions handle connection area\n");
