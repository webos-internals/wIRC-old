...
void cleanup(int sig) {
	syslog(LOG_ALERT, "Cleanup caused by: %d", sig);
	autoPing = 0;
...
int main(int argc, char *argv[]) {
	...
	if (isDir("/media/internal/wirc")) {
		...
		if (!isDir("/media/internal/wirc/backlogs")) {
			syslog(LOG_ALERT, "!/media/internal/wirc/backlogs");
		}
		mkdir("/media/internal/wirc/downloads", S_IRWXU);
		if (!isDir("/media/internal/wirc/downloads")) {
			syslog(LOG_ALERT, "!/media/internal/wirc/downloads");
		}
	} else {
		syslog(LOG_ALERT, "!/media/internal/wirc");
	}
	...
	if (ret == PDL_NOERROR) {
		syslog(LOG_ALERT, "JS handler registration complete");
		plugin_start();
	} else {
